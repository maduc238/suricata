...
DiameterMessageHeader ReadDiameterHeaderData(const uint8_t *data, uint32_t data_len) {
        ...
        const uint8_t *input, uint32_t input_len, uint8_t *rdir)
{
    ...
    if (input[0] == 0x01 && (input[4] & 0x01) == 0 ) {
        ...
        StreamSlice stream_slice, void *local_data)
{
    ...
    if (unlikely(tx->request_buffer == NULL)) {
        SCLogNotice("Fail to  copy request packet");
        goto end;
    }
...
static int DiameterGetStateProgress(void *txv, uint8_t direction)
{
    ...
    SCLogNotice("Transaction progress requested for tx ID %"PRIu32
        ", direction=0x%02x", tx->tx_id, direction);

    if ((direction & STREAM_TOCLIENT) && tx->response_done == 1) {
        return 1;
    }
...
{
    DiameterTransaction *d_state = (DiameterTransaction *)vtx;
